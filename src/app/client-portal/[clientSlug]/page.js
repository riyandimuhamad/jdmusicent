                  ...
                  formData.galleryItems.map((item, idx) => (
                    <div key={idx} className="flex space-x-3 items-center bg-black/20 p-3 rounded-xl border border-white/5">
                      <input 
                        type="url" 
                        value={item.src} 
                        onChange={(e) => handleGallerySrcChange(idx, e.target.value)} 
                        placeholder="Link Gambar URL (https://...)" 
                        className="flex-1 px-4 py-2 rounded-lg border border-white/10 bg-black/40 text-white text-sm focus:outline-none focus:border-gold"
                      />
                      {item.src && (
                        <div className="w-10 h-10 rounded border border-white/10 overflow-hidden bg-white/5 shrink-0 flex-none">
                          <img src={item.src} alt="" className="w-full h-full object-cover" onError={(e) => e.target.style.display='none'} />
                        </div>
                      )}
                      <button type="button" onClick={() => removeGalleryItem(idx)} className="p-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-lg transition-colors">
